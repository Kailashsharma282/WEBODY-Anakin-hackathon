    ...
    <div className="min-h-screen bg-background text-slate-100 flex flex-col font-sans">
      ...
      <header className="sticky top-0 z-40 w-full glass-panel border-b border-surface-border bg-background/90 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          ...
          {/* Right Controls: Mode & RUN THE FUTURE */}
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded bg-slate-900 border border-slate-800 text-[10px] font-mono text-slate-300">
              <span
                className={`w-2 h-2 rounded-full ${
                  settings.ANAKIN_API_KEY_CONFIGURED ? "bg-hud-emerald" : "bg-hud-amber"
                }`}
              />
              <span>{settings.ANAKIN_API_KEY_CONFIGURED ? "LIVE ANAKIN" : "DEMO RELIABLE"}</span>
            </div>
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-hud-cyan via-teal-400 to-hud-cyan text-slate-950 font-mono font-bold text-xs tracking-wider shadow-lg shadow-hud-cyan/25 hover:opacity-95 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50"
            >
              <Play className={`w-3.5 h-3.5 fill-current ${isDemoRunning ? "animate-spin" : ""}`} />
              {isDemoRunning ? "SIMULATING..." : "RUN THE FUTURE"}
            </button>
          </div>
        </div>
